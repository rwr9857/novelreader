...
<head>
	...
	<script type="text/javascript" src="${root}/javascript/manager/memberModify.js"></script>
	...
	<div style="margin-left: 265px; margin-top: 0px; border: 1px solid black; height: 1100px; background: rgb(234, 240, 253);">
		...
		<c:if test="${count > 0}">
			<c:forEach var="memberDto" items="${memberList}">
				<div class="resultTextpage">
					...
					<button onclick="memberUpdate('${root}','${memberDto.m_num}')">수정</button>
				</div>
			</c:forEach>
		</c:if>
		...
		<div align="center">
			...
			<c:if test="${startPage > pageBlock}">
				<a href="${root}/manager/memberModify.do?pageNumber=${startPage - pageBlock}">[이전]</a>
			</c:if>

			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<a href="${root}/manager/memberModify.do?pageNumber=${i}">[${i}]</a>
			</c:forEach>

			<c:if test="${endPage < pageCount}">
				<a href="${root}/manager/memberModify.do?pageNumber=${startPage + pageBlock}">[다음]</a>
			</c:if>
		</div>
